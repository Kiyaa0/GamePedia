<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                @foreach($game['genres'] ?? [] as $genre)
                    <span class="bg-red-600/20 text-red-400 text-xs px-3 py-1 rounded-full border border-red-600/30">{{ $genre['name'] }}</span>
                @endforeach
            </div>

            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                <div class="bg-gray-900 border border-[#2a2a2a] rounded-lg p-3 text-center">
            ...
            @if($pc && !empty($pc['requirements']['minimum']))
                <div class="bg-gray-900 border border-[#2a2a2a] rounded-lg p-5 mb-6">
                    <div class="flex items-center gap-2 mb-3">
                        <span class="inline-block w-[14px] h-[1px] bg-[#e21c1c]"></span>
                        <h3 class="font-semibold text-red-400">PC Requirements</h3>
                    </div>
        ...
        {{-- Kanan --}}
        <div>
            <div class="bg-gray-900 border border-[#2a2a2a] rounded-lg p-5 sticky top-24 space-y-4">
                    ...
                    @auth
                        ...
                        @else
                            <form method="POST" action="{{ route('wishlist.add-from-game', $game['id']) }}">
                                @csrf
                                <button class="w-full bg-red-500 hover:bg-red-600 py-2 rounded-md text-sm font-medium transition">
                                    Tambah ke Wishlist
                                </button>
                            </form>
                        @endif
                    @else
                        <a href="{{ route('login') }}" class="block w-full text-center bg-red-500 hover:bg-red-600 py-2 rounded-md text-sm transition">Login untuk Wishlist</a>
                    @endauth
                </div>

                {{-- Game Info --}}
                <div class="border-t border-[#2a2a2a] pt-4 space-y-2 text-sm">
