...
@section('content')
    ...
    <section class="py-10 md:py-16 px-4 md:px-0">
        <div class="flex items-center justify-between mb-6 md:mb-8">
            <div class="flex items-center gap-3">
                <span class="inline-block w-4 md:w-5 h-px bg-[#E51920]"></span>
                <h2 class="text-sm md:text-lg font-bold tracking-[0.15em] text-white uppercase">Trending Now</h2>
            </div>
            <a href="{{ route('games.index') }}" class="text-[10px] md:text-xs text-gray-500 hover:text-white uppercase tracking-widest transition">View All →</a>
        </div>
                ...
                <a href="{{ route('games.show', $game['id']) }}" class="group block">
                    <div class="relative aspect-video bg-[#1a1a1a] overflow-hidden rounded-lg"
                        ...
                        <img src="{{ $game['background_image'] ?? 'https://via.placeholder.com/400x225/1a1a1a/666?text=No+Image' }}"
                            ...
                            loading="lazy">
                        <span class="absolute bottom-2 md:bottom-3 left-2 md:left-3 text-base md:text-lg font-black text-[#E51920]">{{ number_format($game['rating'], 1) }}</span>
                    </div>
                    <div class="mt-3 md:mt-4">
                        <h3 class="text-xs md:text-sm font-bold tracking-[0.15em] text-white uppercase">{{ $game['name'] }}</h3>
    ...
    @if ($newsItems->isNotEmpty())
        ...
        <section class="pb-10 md:pb-16 px-4 md:px-0">
            <div class="flex items-center justify-between mb-4 md:mb-6">
                <div class="flex items-center gap-2">
                    <span class="inline-block w-1 h-4 md:h-5 bg-[#E51920]"></span>
                    <h2 class="text-white font-bold uppercase tracking-widest text-xs md:text-base">BERITA GAME</h2>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                ...
                <div class="bg-[#1a1a1a] rounded-lg relative overflow-hidden group">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-3 md:p-5 z-10">
                        <div class="flex items-center gap-2 mb-1 md:mb-2">
                            <span class="inline-block w-1 h-2 md:h-3 bg-[#E51920]"></span>
                            <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{{ $featuredGame['game_name'] }}</span>
                        </div>
                        <h3 class="text-sm md:text-lg font-bold text-white leading-tight line-clamp-2">{{ $featuredNews['title'] }}</h3>
                        <p class="text-[10px] md:text-xs text-gray-500 mt-1 md:mt-2">{{ \Carbon\Carbon::createFromTimestamp($featuredNews['date'])->diffForHumans() }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-3 md:space-y-4">
                    @foreach ($restNews as $news)
                        <a href="{{ $news['url'] }}" target="_blank" rel="noopener noreferrer" class="flex items-start gap-3 md:gap-4 group">
                            <div class="w-12 h-12 md:w-16 md:h-16 rounded flex-shrink-0 overflow-hidden bg-gray-800">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs md:text-sm font-semibold text-white group-hover:text-[#E51920] transition leading-snug line-clamp-2">{{ $news['title'] }}</p>
                                <p class="text-[10px] md:text-xs text-gray-600 mt-1">{{ \Carbon\Carbon::createFromTimestamp($news['date'])->diffForHumans() }}</p>
                            </div>
                        </a>
                        @if (! $loop->last)
    ...
    @if ($genres->isNotEmpty())
        <section class="pb-16 md:pb-20 px-4 md:px-0">
            <div class="flex items-center gap-2 mb-4 md:mb-6">
                <span class="inline-block w-1 h-4 md:h-5 bg-[#E51920]"></span>
                <h2 class="text-white font-bold uppercase tracking-widest text-xs md:text-base">BROWSE BY GENRE</h2>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                @foreach ($genres as $genre)
                    ...
                    <a href="{{ route('games.index', ['genre' => $genre['slug']]) }}"
                        class="bg-[#1a1a1a] border border-transparent rounded-lg flex flex-col items-center justify-center p-4 md:p-8 hover:border-[#E51920] hover:-translate-y-1 hover:scale-[1.02] transition-all duration-300 {{ $isFeatured ? 'md:col-span-2 md:row-span-2' : '' }}">
                        {!! $icon !!}
                        <span class="text-white font-bold uppercase text-[10px] md:text-sm">{{ $genre['name'] }}</span>
                    </a>
                @endforeach
            </div>
        </section>
